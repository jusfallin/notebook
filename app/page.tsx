...
export default function Page() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setState({
        entries: Array.isArray(parsed.entries) && parsed.entries.length ? parsed.entries : initialEntries,
        dedication: parsed.dedication || DEFAULT_DEDICATION,
        dedicationPhoto: parsed.dedicationPhoto || '',
        coverOpened: false,
      });
    } catch {}
  }, []);
  ...
  function startTurn(target: number) {
    ...
    playPageRustle(); setTurn({ from: page, to: target, direction });
  ...
  function updateActive(patch: Partial<Entry>) {
    const index = page - 2; const entry = state.entries[index]; if (!entry) return;
  ...
  function duplicateEntry() { const entry = state.entries[page - 2]; if (!entry) return; const clone = { ...entry, id: uid(), date: todayString() }; patchState(s => ({ ...s, entries: [...s.entries, clone] })); setPage(entryPage(state.entries.length)); setMode('studio'); setEdit(true); }
  ...
  function confirmDelete() {
    ...
    const index = state.entries.findIndex(e => e.id === deleteTarget); if (index < 0) { setDeleteTarget(null); return; }
    patchState(s => ({ ...s, entries: s.entries.filter(e => e.id !== deleteTarget) }));
    setPage(Math.max(2, Math.min(page, state.entries.length)));
  ...
  function renderDedicationPage() {
    const canEdit = mode === 'studio' && edit && page === 1;
    return <section className="opening-page dedication-page spread">
      <div className="dedication-ornament">♡</div>
      <div className="dedication-heading">TO MY LOVE <span>♡</span></div>
      <div className="dedication-photo-frame">
        {state.dedicationPhoto ? <img src={state.dedicationPhoto} alt="Our photo"/> : canEdit ? <label className="dedication-upload" htmlFor="dedication-photo"><ImagePlus size={25}/><strong>Add our photo</strong><small>Choose a picture of us</small></label> : <div className="dedication-placeholder"><ImagePlus size={25}/><span>Our photo goes here</span><small>Add it from the Writing Desk</small></div>}
        {canEdit && state.dedicationPhoto && <button type="button" className="dedication-remove-photo" onClick={removeDedicationPhoto} aria-label="Remove photo"><Trash2 size={14}/></button>}
        {canEdit && <input id="dedication-photo" type="file" accept="image/*" hidden onChange={e => handleDedicationImage(e.target.files?.[0])}/>} 
      </div>
      <div className="dedication-rule"><i/><span>♡</span><i/></div>
      {canEdit ? <textarea className="dedication-editor" value={state.dedication} onChange={e => updateDedication({ dedication: e.target.value })} aria-label="Lovely lines"/> : <div className="dedication-lines">{state.dedication.split('\n').map((line, i) => <span key={`${i}-${line}`}>{line || '\u00a0'}</span>)}</div>}
      <div className="dedication-sign">— with love, always —</div>
      {canEdit && <div className="dedication-edit-note">Write your own lines above, add your photo, then save the page. ♡</div>}
      <div className="page-footer dedication-footer"><span>Our first page · before the memories</span><span className="footer-heart">♡</span></div>
    </section>;
  }
  ...
  function renderEntry(index: number) {
    if (index === 1) return renderDedicationPage();
    const entryIndex = index - 2; const entry = entryIndex >= 0 ? state.entries[entryIndex] : null; if (!entry) return null;
