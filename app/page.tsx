...
import { BookOpen, ChevronLeft, ChevronRight, ImagePlus, Music2, Volume2, VolumeX, Heart, Sparkles, CalendarDays, Trash2, Camera, Plus, X, PenLine, Eye, LayoutDashboard, Save, ArrowRight } from 'lucide-react';
...
export default function Page() {
  ...
  function patchState(updater: (s: NotebookState) => NotebookState) {
    setState(updater);
    setDirty(true);
  }
  ...
  function next() {
    ...
    if (mode === 'reader') startTurn(page + 1);
  }

  function prev() {
    if (page === 1) { closeToCover(); return; }
    if (page === 2) { setPage(1); return; }
    if (mode === 'reader' && page >= 3) startTurn(page - 1);
  }

  function openCover() {
    if (openingCover || page !== 0) return;
    setOpeningCover(true);
    setTimeout(() => { setPage(1); setOpeningCover(false); }, 650);
  }
  ...
  function addEntry() {
    ...
    patchState(s => ({ ...s, entries: [...s.entries, nextEntry] }));
    setPage(entryPage(state.entries.length)); setMode('studio'); setEdit(true);
  }

  function duplicateEntry() {
    const entry = state.entries[page - 3]; if (!entry) return;
    const clone = { ...entry, id: uid(), date: todayString() };
    patchState(s => ({ ...s, entries: [...s.entries, clone] }));
    setPage(entryPage(state.entries.length)); setMode('studio'); setEdit(true);
  }
  ...
  function confirmDelete() {
    ...
    setPage(Math.max(3, Math.min(page, 2 + state.entries.length - 1)));
  ...
  if (mode === 'home' && page === 1) return <main className="scene desktop-notebook opening-scene"><div className="ambient"/><section className="desk physical-desk"><div className="book-stage opening-book-stage"><article className="paper paper-base opening-paper">{renderOpeningPage()}</article></div></section></main>;
